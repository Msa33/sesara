...
function UnProduit (props: ProduitProps){
  const AddToCart = (nomProduit: string) => {
    console.log('Produit ajouté au panier :', + nomProduit);
    // implementation d'ajout au panier
      ...
      <div className="relative group shadow-xl/20">
        <div className="overflow-hidden aspect-w-1 aspect-h-1 relative">
          ...
          <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition duration-300">
            <button
              onClick={() => AddToCart(props.nomProduit)}
        ...
        <div className="flex items-start justify-center mt-4 space-x-4">
          <div>
            <h3 className="text-xs font-bold text-gray-900 sm:text-sm md:text-base">{props.nomProduit}</h3>
          ...
          <div className="text-center">
            <p className="text-xs font-bold text-gray-900 sm:text-sm md:text-base">{props.prix}</p>
